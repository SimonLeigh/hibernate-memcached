...
public class MemcachedRegionFactory implements RegionFactory {
    private static final long serialVersionUID = -3512428231757497766L;
    ...
    protected MemcacheClientFactory getMemcachedClientFactory(Config config) {
        ...
        try {
            ...
            throw new CacheException("Unable to find PropertiesHelper constructor for factory class ["
                    + factoryClassName + "]", e);
        }
    ...
    public NaturalIdRegion buildNaturalIdRegion(String name, Properties properties, CacheDataDescription metadata) {

        return new NaturalIdRegionImpl(name, metadata, settings);
    }
}
