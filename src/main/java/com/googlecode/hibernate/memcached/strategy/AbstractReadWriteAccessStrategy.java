 ...
 *              on: $Date: 29 May 2013 16:55:56 $
 *        Filename: $RCSfile: AbstractReadWriteAccessStrategy.java $
 * 
 */
...
abstract class AbstractReadWriteAccessStrategy extends BaseRegionAccessStrategy {
    private static final CoreMessageLogger LOG = Logger.getMessageLogger(CoreMessageLogger.class,
            AbstractReadWriteAccessStrategy.class.getName());
    ...
    /**
     * Returns <code>null</code> if the item is not readable. Locked items are
     * not readable, nor are items created after the start of this transaction.
     */
    @Override
    ...
    public final boolean putFromLoad(Object key, Object value, long txTimestamp, Object version,
        ...
        try {
            ...
            if (writeable) {
                LOG.debugf("putting key[%s] -> value[%s] into region[%s] success", key, value, getInternalRegion()
                        .getName());
                getInternalRegion().put(key, new Item(value, version, getInternalRegion().nextTimestamp()));
                return true;
            } else {
                LOG.debugf("putting key[%s] -> value[%s] into region[%s] fail due to it is unwriteable", key, value,
    ...
    public final SoftLock lockItem(Object key, Object version) throws CacheException {
        ...
        try {
            ...
            final Lock lock = (item == null) ? new Lock(timeout, uuid, nextLockId(), version) : item.lock(timeout,
                    uuid, nextLockId());
    ...
    protected static interface Lockable {
        ...
        /**
         * Returns <code>true</code> if the enclosed value can be read by a
         * transaction started at the given time.
         */
        public boolean isReadable(long txTimestamp);

        /**
         * Returns <code>true</code> if the enclosed value can be replaced with
         * one of the given version by a transaction started at the given time.
         */
        ...
        /**
         * Returns <code>true</code> if the given lock can be unlocked using the
         * given SoftLock instance as a handle.
         */
        public boolean isUnlockable(SoftLock lock);

        /**
         * Locks this entry, stamping it with the UUID and lockId given, with
         * the lock timeout occuring at the specified time. The returned Lock
         * object can be used to unlock the entry in the future.
         */
    ...
    protected final static class Item implements Serializable, Lockable {
        ...
        /**
         * Creates an unlocked item wrapping the given value with a version and
         * creation timestamp.
         */
    ...
    protected final static class Lock implements Serializable, Lockable, SoftLock {
        ...
        /**
         * Returns true if this Lock has been concurrently locked by more than
         * one transaction.
         */
